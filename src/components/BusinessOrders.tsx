...
import OrderBoard, { type OrderBoardStatusGroup } from "@/components/orders/OrderBoard";
...
export default function BusinessOrders() {
  ...
  const statusGroups = useMemo<OrderBoardStatusGroup[]>(
    () => [
      {
        ...
        items: [
          { label: t("orders.status.tempSave"), code: "BUY_TEMP_SAVE", progressStatusParam: "P_TEMPSAVE" },
          { label: t("orders.status.purchaseQuote"), code: "BUY_EST", progressStatusParam: "P_QUOTE" },
          { label: t("orders.status.purchasePaymentPending"), code: "BUY_PAY_WAIT", progressStatusParam: "P_PENDING" },
          { label: t("orders.status.purchasePaymentComplete"), code: "BUY_PAY_DONE", progressStatusParam: "P_PAY_COMPLETE" },
          { label: t("orders.status.purchasing"), code: "BUYING", progressStatusParam: "P_AU_PURCHASING" },
          { label: t("orders.status.problemProduct"), code: "PROBLEM_PRODUCT", progressStatusParam: "P_MA_PROBLEM" },
          { label: t("orders.status.purchaseComplete"), code: "BUY_COMPLETE", progressStatusParam: "P_PUR_COMPLETE" },
        ],
      },
      {
        ...
        color: "text-blue-500",
        items: [
          { label: t("orders.status.centerArrivalExpected"), code: "WH_ARRIVE_EXPECTED", progressStatusParam: "IO_ARRIVE_EXPECTED" },
          { label: t("orders.status.localDeliveryDelay"), code: "LOCAL_DELAY", progressStatusParam: "IO_DELAY" },
          { label: t("orders.status.warehouseInProgress"), code: "WH_IN_PROGRESS", progressStatusParam: "IO_PROGRESS" },
          { label: t("orders.status.warehouseInComplete"), code: "WH_IN_DONE", progressStatusParam: "IO_WARE_COMPLETE" },
          { label: t("orders.status.shipmentPaymentPending"), code: "SHIP_PAY_WAIT", progressStatusParam: "IO_SHIP_PAY_PENDING" },
          { label: t("orders.status.shipmentPaymentComplete"), code: "SHIP_PAY_DONE", progressStatusParam: "IO_SHIP_PAY_COMPLETE" },
          { label: t("orders.status.shipmentPending"), code: "WH_SHIP_WAIT", progressStatusParam: "IO_SHIP_PENDING" },
          { label: t("orders.status.shipmentComplete"), code: "WH_SHIPPED", progressStatusParam: "IO_SHIP_COMPLETE" },
          { label: t("orders.status.additionalCostPaymentPending"), code: "ADD_COST_PAY_WAIT", progressStatusParam: "IO_COST_PENDING" },
          { label: t("orders.status.additionalCostPaymentComplete"), code: "ADD_COST_PAY_DONE", progressStatusParam: "IO_COST_COMPLETE" },
        ],
      },
      {
        ...
        color: "text-red-500",
        items: [
          { label: t("orders.status.errorWarehouse"), code: "ERR_IN", progressStatusParam: "E_ERROR" },
          { label: t("orders.status.userRefundRequest"), code: "USER_REFUND_REQ", progressStatusParam: "E_CUSTOMER_RETURN_REQ" },
          { label: t("orders.status.userRefundProcessing"), code: "USER_REFUND_ING", progressStatusParam: "E_CUSTOMER_REFUND_PROGRESS" },
          { label: t("orders.status.userRefundComplete"), code: "USER_REFUND_DONE", progressStatusParam: "E_CUSTOMER_REFUND_COMPLETED" },
          { label: t("orders.status.platformRefundRequest"), code: "PLAT_REFUND_REQ", progressStatusParam: "E_PLATFORM_REFUND_REQ" },
          { label: t("orders.status.platformRefundProcessing"), code: "PLAT_REFUND_ING", progressStatusParam: "E_PLATFORM_REFUND_IN_PROGRESS" },
          { label: t("orders.status.platformRefundComplete"), code: "PLAT_REFUND_DONE", progressStatusParam: "E_PLATFORM_REFUND_COMPLETED" },
          { label: t("orders.status.finalRefundRequest"), code: "FINAL_REFUND_REQ", progressStatusParam: "E_FINAL_REFUND_REQ" },
          { label: t("orders.status.finalRefundProcessing"), code: "FINAL_REFUND_ING", progressStatusParam: "E_FINAL_REFUND_PROGRESS" },
          { label: t("orders.status.finalRefundComplete"), code: "FINAL_REFUND_DONE", progressStatusParam: "E_FINAL_REFUND_COMPLETED" },
          { label: t("orders.status.orderDisposal"), code: "ORDER_DISPOSAL", progressStatusParam: "E_ORDER_CANCELLED" },
          { label: t("orders.status.shipmentHold"), code: "HOLD", progressStatusParam: "E_SHIPMENT_HOLD" },
        ],
      },
    ],
    [t],
  ...
  return (
    <OrderBoard
      ...
      orders={[]}
      ordersDataSource="manualSearchApi"
