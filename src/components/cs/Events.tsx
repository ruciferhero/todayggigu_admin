...
const Events: React.FC = () => {
  ...
  const handleExport = () => {};
  const handleAdd = () => {};
  const handleView = (record: EventItem) => { void record; };
  const handleSearch = () => {};
  ...
  const headerOptions = [
    { value: "join-membership", label: t("cs.event.header.joinMembership") },
    { value: "apply-shipping-agency", label: t("cs.event.header.applyShippingAgency") },
    { value: "combined-packaging", label: t("cs.event.header.combinedPackaging") },
    { value: "overseas-return", label: t("cs.event.header.overseasReturn") },
    { value: "incoming-outgoing-delivery", label: t("cs.event.header.incomingOutgoingDelivery") },
    { value: "service-cost", label: t("cs.event.header.serviceCost") },
    { value: "customs-clearance", label: t("cs.event.header.customsClearance") },
    { value: "compensation", label: t("cs.event.header.compensation") },
  ];

  const tabItems = [{ key: "all", label: t("cs.event.tab.all") }];
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("cs.event.number")}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("cs.event.fieldTitle")}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("cs.event.author")}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("cs.event.createdAt")}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("cs.event.isHeader")}</label>
            <select value={searchIsHeader} onChange={(e) => setSearchIsHeader(e.target.value)} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
              <option value="">{t("cs.event.isHeader")}</option>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("cs.event.content")}</label>
      ...
      <div className="flex justify-end space-x-2">
        ...
        <button onClick={handleExport} className="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
          <Download className="w-4 h-4 mr-1" />{t("cs.event.export")}
        </button>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase w-20">{t("cs.event.number")}</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase w-96">{t("cs.event.fieldTitle")}</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase w-36">{t("cs.event.author")}</th>
              <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase w-24">{t("cs.event.views")}</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase w-44">{t("cs.event.createdAt")}</th>
            </tr>
          </thead>
