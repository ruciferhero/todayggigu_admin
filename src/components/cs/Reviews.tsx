...
const Reviews: React.FC = () => {
  ...
  const [data] = useState<ReviewItem[]>([]);
  ...
  const handleExport = () => {};
  const handleAdd = () => {};
  const handleView = (record: ReviewItem) => { void record; };
  const handleSearch = () => {};
  ...
  const headerOptions = [
    { value: "join-membership", label: t("cs.reviews.header.joinMembership") },
    { value: "apply-shipping-agency", label: t("cs.reviews.header.applyShippingAgency") },
    { value: "combined-packaging", label: t("cs.reviews.header.combinedPackaging") },
    { value: "overseas-return", label: t("cs.reviews.header.overseasReturn") },
    { value: "incoming-outgoing-delivery", label: t("cs.reviews.header.incomingOutgoingDelivery") },
    { value: "service-cost", label: t("cs.reviews.header.serviceCost") },
    { value: "customs-clearance", label: t("cs.reviews.header.customsClearance") },
    { value: "compensation", label: t("cs.reviews.header.compensation") },
  ];

  const tabItems = [{ key: "all", label: t("cs.reviews.tab.all") }];
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("cs.reviews.number")}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("cs.reviews.fieldTitle")}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("cs.reviews.author")}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("cs.reviews.createdAt")}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("cs.reviews.isHeader")}</label>
            <select value={searchIsHeader} onChange={(e) => setSearchIsHeader(e.target.value)} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
              <option value="">{t("cs.reviews.isHeader")}</option>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("cs.reviews.content")}</label>
      ...
      <div className="flex justify-end space-x-2">
        ...
        <button onClick={handleExport} className="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
          <Download className="w-4 h-4 mr-1" />{t("cs.reviews.export")}
        </button>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase w-20">{t("cs.reviews.number")}</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase w-96">{t("cs.reviews.fieldTitle")}</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase w-36">{t("cs.reviews.author")}</th>
              <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase w-24">{t("cs.reviews.views")}</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase w-44">{t("cs.reviews.createdAt")}</th>
            </tr>
          </thead>
