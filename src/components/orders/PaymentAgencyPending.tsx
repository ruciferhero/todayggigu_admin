...
export default function PaymentAgencyPending() {
  const { t } = useLocale();
  const [selectedCode, setSelectedCode] = useState<string | null>(null);
  const [selectedLabel, setSelectedLabel] = useState(t("orders.paymentAgency.pendingDefault"));
  const [selectedRowKeys] = useState<string[]>([]);

  const paymentStatuses = [
    { label: t("orders.paymentAgency.waitingForPayment"), count: 0, code: "PA_WAIT", color: "text-orange-500", icon: <ShoppingCart className="w-5 h-5" /> },
    { label: t("orders.paymentAgency.depositCompleted"), count: 0, code: "PA_DEPOSIT", color: "text-green-500", icon: <ShoppingCart className="w-5 h-5" /> },
    { label: t("orders.paymentAgency.paymentAtPurchase"), count: 0, code: "PA_PURCHASE", color: "text-blue-500", icon: <Warehouse className="w-5 h-5" /> },
    { label: t("orders.paymentAgency.refundRequest"), count: 0, code: "PA_REFUND", color: "text-red-500", icon: <Warehouse className="w-5 h-5" /> },
  ];
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <h1 className="text-xl font-bold text-gray-900">{t("orders.paymentAgency.pendingTitle")}</h1>
        ...
        {selectedCode && (<button onClick={() => { setSelectedCode(null); setSelectedLabel(t("orders.paymentAgency.pendingDefault")); }} className="flex items-center gap-1.5 px-3 py-1.5 text-sm border border-gray-300 rounded-lg hover:bg-gray-50"><X className="w-4 h-4" />{t("orders.action.viewAll")}</button>)}
      </div>
      ...
      <div className="flex items-center justify-between">
        <div>{selectedRowKeys.length > 0 && (<button className="h-8 px-3 text-sm bg-green-600 text-white rounded-md flex items-center gap-1.5"><Check className="w-3.5 h-3.5" />{t("orders.action.confirm")} ({selectedRowKeys.length})</button>)}</div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-x-auto">
        <table className="w-full text-sm">
          <thead><tr className="bg-gray-50 border-b border-gray-200">
            ...
            <th className="px-3 py-2 text-xs font-medium text-gray-500">{t("orders.common.applicationNumber")}</th>
            <th className="px-3 py-2 text-xs font-medium text-gray-500">{t("orders.common.membershipCode")}</th>
            <th className="px-3 py-2 text-xs font-medium text-gray-500">{t("orders.common.amountKrw")}</th>
            <th className="px-3 py-2 text-xs font-medium text-gray-500">{t("orders.common.amountCny")}</th>
            <th className="px-3 py-2 text-xs font-medium text-gray-500">{t("orders.common.status")}</th>
            <th className="px-3 py-2 text-xs font-medium text-gray-500">{t("orders.common.date")}</th>
            <th className="px-3 py-2 text-xs font-medium text-gray-500">{t("orders.common.actions")}</th>
          </tr></thead>
          <tbody>{orders.length === 0 ? (<tr><td colSpan={9} className="py-20 text-center text-gray-400">{t("page.underConstruction")}</td></tr>) : null}</tbody>
        </table>
      </div>
    </div>
  );
