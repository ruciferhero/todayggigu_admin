...
export default function RocketOrders() {
  ...
  const statusGroups: OrderBoardStatusGroup[] = [
    ...
    { title: t("orders.status.inOut"), icon: <Warehouse className="w-4 h-4" />, color: "text-blue-500", items: [
      { label: t("orders.status.centerArrivalExpected"), code: "IO_ARRIVE_EXPECTED" }, { label: t("orders.status.localDeliveryDelay"), code: "IO_DELAY" }, { label: t("orders.status.warehouseInProgress"), code: "IO_PROGRESS" },
      { label: t("orders.status.warehouseInComplete"), code: "IO_WARE_COMPLETE" }, { label: t("orders.status.shipmentPaymentPending"), code: "IO_SHIP_PAY_PENDING" }, { label: t("orders.status.shipmentPending"), code: "IO_SHIP_PENDING" },
